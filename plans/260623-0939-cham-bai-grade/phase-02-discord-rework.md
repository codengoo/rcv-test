# Phase 02 — Discord rework: gỡ hóa đơn, thêm `/cham-bai`

**Goal:** `DiscordService` không còn xử lý `messageCreate`/hóa đơn/dedup. Đăng ký thêm `/cham-bai`, xử lý: tải ảnh → `GradeService.grade` → append 1 dòng vào Sheet → reply embed. `receipt.schema.ts` bị xóa.

## 1. Files chạm vào
| File | Action |
|---|---|
| src/discord/discord.service.ts | MODIFY (rewrite phần lớn) |
| src/discord/discord.module.ts | MODIFY |
| src/discord/receipt.schema.ts | DELETE |

## 2. src/discord/discord.service.ts (nội dung mới đầy đủ)
```ts
import {
  Injectable,
  Logger,
  ...
  private async registerCommands(): Promise<void> {
    try {
      ...
      const chamBai = new SlashCommandBuilder()
        .setName('cham-bai')
        .setDescription('Chấm bài làm của thí sinh từ ảnh, ghi điểm vào Google Sheet')
        .addAttachmentOption((o) =>
          o.setName('file').setDescription('Ảnh bài làm (trang 1)').setRequired(true),
        )
        .addStringOption((o) =>
        ...
        .addStringOption((o) =>
          o.setName('sdt').setDescription('Số điện thoại bố mẹ').setRequired(true),
        )
        .addStringOption((o) =>
...
## 3. src/discord/discord.module.ts (mới)
```ts
...
@Module({
  imports: [GoogleSheetsModule, QuizModule, GradeModule],
  providers: [DiscordService],
})
export class DiscordModule {}
```
> Bỏ `GeminiModule` khỏi import của DiscordModule: `DiscordService` không còn inject `GeminiService` trực tiếp (Quiz/Grade tự import GeminiModule).

## 4. Xóa file
- DELETE `src/discord/receipt.schema.ts` (không còn dùng).

## 5. Encapsulation / wiring notes
- `DiscordService` gọi domain qua public method: `quiz.solveAndSave`, `grade.grade`, `sheets.appendRow` — không import schema nội bộ của grade ngoài type `GradeImage`.
- discord.js: **required option phải đứng trước optional option** trong cùng command → đã sắp `file, hoten, bome, sdt, lop` (required) trước `file2..file5` (optional).
- Chỉ giữ intent `GatewayIntentBits.Guilds`. Sau khi gỡ, có thể tắt **Message Content Intent** trong Developer Portal (không bắt buộc nhưng nên — ghi ở README phase 03).
- `this.channelId`, `seen`, `seenQueue`, `SEEN_CAP`, `RECEIPT_PROMPT`, `tryExtractReceipt`, `handleMessage`, `isDuplicate`, `remember` đều bị gỡ.

## 6. Acceptance criteria
- [ ] `npm run typecheck` pass (không còn tham chiếu `receipt.schema`, `GeminiService` trong DiscordService).
- [ ] `npm run build` thành công.
- [ ] Server boot không lỗi DI (`GradeModule` resolve được).
- [ ] `src/discord/receipt.schema.ts` đã bị xóa; `grep -r receiptSchema src` không còn kết quả.
- [ ] (Manual, nếu có token thật) `/cham-bai` xuất hiện trong server; gửi ảnh bài làm A01 → bot reply điểm dạng `n/12` và sheet có 1 dòng mới đúng 6 cột.
- [ ] (Regression) `/add-quiz` vẫn hoạt động (tải PDF/DOCX → lưu `.md`).

## 7. Out of scope (phase này)
- Không sửa env/README (Phase 03).
- Không đổi `GeminiService`/`GoogleSheetsService`.

## 8. Commit message dự kiến
```
feat(discord): replace message listener with /cham-bai grading command

Remove messageCreate handler, receipt extraction, dedup and message-read
intents. Add /cham-bai: collects student answer images + thí sinh info,
calls GradeService, appends [họ tên, bố mẹ, sđt, lớp, điểm, link ảnh] to
the sheet and replies an embed. Delete receipt.schema.ts; keep /add-quiz.
Client now needs only the Guilds intent.
```
